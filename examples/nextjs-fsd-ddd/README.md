...
- **DDD entity identity** — `src/entities/order/model/order.ts`
  ...
  `@forge/forge/ddd-entity-id` rule enforces that anything exported
...
- **Clean Architecture domain isolation** — the `entities/` layer is
  ...
  `@forge/forge/clean-arch-domain-isolation`.
...
```bash
pnpm seed:examples
pnpm --filter @forge/example-nextjs-fsd-ddd forge:check
```

## Demo violation branches

- `demo/ddd-entity-no-id` — strips the `id` field from `Order`,
  expected to fail `@forge/forge/ddd-entity-id`.
