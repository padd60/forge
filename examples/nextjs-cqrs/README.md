# @forge-kit-dev/example-nextjs-cqrs
...
- `src/entities/cart/` — **read model.** All exports are `readonly`
  ...
  live here; the `@forge-kit-dev/forge/cqrs-layer-role` rule enforces that.
...
```bash
pnpm seed:examples
pnpm --filter @forge-kit-dev/example-nextjs-cqrs forge:check
```
...
- `demo/cqrs-write-in-entity` — adds `export function createCart(...)`
  ...
  `@forge-kit-dev/forge/cqrs-layer-role` (entities must not expose commands).
